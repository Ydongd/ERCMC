...
class Model(nn.Module):
    def __init__(self, args, tokenizer, position_mode, window, d_inner=2048, n_layers=2, n_head=8, dropout=0.1, n_position=120):
        ...
        self.W_2 = nn.Linear(self.plm.config.hidden_size * 3, self.plm.config.hidden_size)
        self.W_3 = nn.Linear(self.plm.config.hidden_size * 3, self.plm.config.hidden_size)
        self.W_4 = nn.Linear(self.plm.config.hidden_size * 3, self.plm.config.hidden_size)
        self.tanh = nn.Tanh()
        ...
        self.x2h_a = nn.Linear(self.plm.config.hidden_size, self.plm.config.hidden_size * 3)
        self.h2h_a = nn.Linear(self.plm.config.hidden_size, self.plm.config.hidden_size * 3)
    ...
    def forward(self, batch_dialog_input_ids, batch_dialog_attention_mask, batch_dialog_generated_input_ids, batch_dialog_generated_attention_mask, batch_dialog_speakers, batch_dialog_label_ids, mode):
        ...
        for j in range(batch_size):
            ...
            hx_a = torch.zeros((1, self.plm.config.hidden_size)).cuda()
            ...
            for i in range(len_d):
                ...
                # Augmentation-aware embedding
                generated_embeds = self.plm(input_ids=torch.tensor(dialog_generated_input_ids[i][:self.window]).cuda(), attention_mask=torch.tensor(dialog_generated_attention_mask[i][:self.window]).cuda())['last_hidden_state'][:,0]
                now_a = 0
                augment_embeds = dialog_embeds[i:i+1]
                augment_embeds = torch.cat((augment_embeds, generated_embeds),dim=0).unsqueeze(0)
                # Encoding
                # [1, u_num, hidden]
                out_context = self.encoder(context_embeds)
                out_speaker = self.encoder(speaker_embeds)
                out_augment = self.encoder(augment_embeds)
                # Obtain embedding corresponding to i
                # [1, hidden]
                out_c = out_context[:, now_c]
                out_s = out_speaker[:, now_s]
                out_a = out_augment[:, now_a]
                ...
                local_a = out_augment[:, 1:].squeeze(0)
                local_c_ = self.W_1(local_c)
                local_s_ = self.W_1(local_s)
                local_a_ = self.W_1(local_a)
                ...
                weight_a = F.softmax(self.tanh(torch.mm(local_a_, origin_a.transpose(0, 1))), dim=0)
                ...
                state_a = torch.sum(torch.mul(weight_a, local_a), dim=0).unsqueeze(0)
                ...
                update_gate = torch.sigmoid(x_upd + h_upd)
                new_gate = torch.tanh(x_new + (reset_gate * h_new))
                hx_a = update_gate * hx_a + (1 - update_gate) * new_gate
                # Final
                out_f = self.W_2(torch.cat((out_c, out_s, out_a), dim=-1)) # [1, hidden]
                gs_f = self.W_3(torch.cat((state_c, state_s, state_a), dim=-1)) # [1,hidden]
                hx_f = self.W_4(torch.cat((hx_c, hx_s, hx_a), dim=-1)) # [1,hidden]
